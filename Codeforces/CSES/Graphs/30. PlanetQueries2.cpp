...
signed main() {
    ...
    */
    ...
    while(q--){
        int u,v; cin>>u>>v;
        u--; v--;
        // first check: if both u and v are part of the same component:
            // take a long jump from both u and v and check the id of the cycle on which you land. If you're landing on different cycles, u and v belongs to different component
        if(cycle_id[parent[u][20]] != cycle_id[parent[v][20]]) {
            cout<<"-1"<<endl;
            continue;
        }

        // case 1: if either u or v are part of cycles
        // this handles both scenarios where both u and v are on cycle & u is on tree and v is on cycle 
        if(cycle_id[u] != -1 or cycle_id[v] != -1){
            // handles case when u is on cycle and v is on tree --> ans not possible in this case 
            if(cycle_id[v] == -1 and cycle_id[u] != -1){
                cout<<"-1"<<endl;
                continue;
            }
            
            int dist = cyclic_dist[u];
            // u lies on tree and v lies on cycle 
            int u_cyc = up(u, cyclic_dist[u]);
            
            // node index of u and v in their cycle
            int uidx = cycles[cycle_id[u_cyc]][u_cyc];
            int vidx = cycles[cycle_id[v]][v];
        
            int cycle_len = cycles[cycle_id[u_cyc]].size();
    
            int diff = uidx <= vidx ? vidx - uidx : cycle_len - (vidx - uidx);
            
            cout<<dist + diff<<endl;

        }
        else if(cycle_id[u] == -1 and cycle_id[v] == -1){   // both u and v are not part of cycles
            if(cyclic_dist[v] > cyclic_dist[u]){
                cout<<"-1"<<endl;
                continue;
            }
            // taking a jump from u of length equal to the difference in their depths should land on v. if not ans does not exist
            if (up(u, cyclic_dist[u] - cyclic_dist[v]) != v){
                cout<<"-1"<<endl;
            }
            else{
                cout<<cyclic_dist[u] - cyclic_dist[v]<<endl;
            }
