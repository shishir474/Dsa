...
int modinv(int b, int c){
    return modularBinaryExponentitation(b,c-2)%c;
}

int geometricSum(int base, int exponent){
    int numerator = (modularBinaryExponentitation(base, exponent+1) - 1 + mod)%mod;
    int denominator_inv = modinv(base[i]-1,mod);           
...
signed main() {
    ...
    forn(i,n){
        int prime, power; cin>>prime>>power;
        base.push_back(prime);
        exponent.push_back(power);
    }   

    // computing no of divisors 
    int num_of_divisors = 1;
    forn(i,n){
        number_of_divisors = (number_of_divisors * (exponent[i] + 1))%mod;
    }

    // computing sum of divisors
    int sum_of_divisors = 1;
    forn(i,n){
       sum_of_divisors = (sum_of_divisors * geometricSum(base[i],exponent[i]))%mod;
    }

    // computing product of divisors -- pending 
