...
struct Node{    // Each node of the seg Tree will store 4things
    int sum, maxPrefix, maxSuffix, maxSumSub;
};
...
void buildTree(int index, int lo, int hi){
    ...
    segTree[index].maxSuffix = max(right.maxSuffix, right.sum + left.maxSuffix);
    segTree[index].maxSumSub = max({left.maxSumSub, right.maxSumSub, left.maxSuffix + right.maxPrefix});
}
...
void update(int index, int lo, int hi, int x, int val){
    if(lo == hi){
        ...
        segTree[index].maxSuffix = val;
        segTree[index].maxSumSub = val;
        return;
    }
    int mid = (lo + hi)/2;
    if(x<=mid){
...
signed main() {
    ...
    while(q--){ 
        ...
        k--;        // 0-based indexing
        arr[k] = x;
        update(0,0,arr.size()-1,k,x);
        cout<<max((int)0,segTree[0].maxSumSub)<<endl;
    }
