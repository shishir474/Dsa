...
int queryTree(int index, int lo, int hi, int l, int r){
    // case 1: if segTree's range lies completely within l and r
    if(lo >= l and hi <= r) return segTree[index];
    // case 2: if completely out of bound
    if(hi < l or lo > r) return 0;
    // case3: partial overlap -> proceed in both directions
    int mid = (lo + hi) >> 1;
...
signed main() {
    ...
    // O(N) to build the segment Tree and O(logn) for the query operation
    buildTree(0,0,n-1); // index, segTree range 
...
// Properties of XOR:
// 	1.	a ^ a = 0 (XOR of the same number cancels out)
// 	2.	a ^ 0 = a (XOR with zero keeps the number unchanged)
// 	3.	XOR is associative and commutative, so order doesn’t matter.
