...
// update func complexity  = O(h), and height in worst case can be O(N) for skewed tree-> thats why giving TLE
// void update(int src, vector<int>& ans, vector<vector<int> >& parent,int val){
//     while(src!=-1){
//         ans[src]+=val;
//         src = parent[src][0];
//     }
// }
int dfs_for_ans(int src, int par, vector<int>& ans){
    for(auto child: adj[src]){
        if(child!=par){
            ans[src]+=dfs_for_ans(child,src,ans);
        }
    }
...
// m paths
    while(q--){
        ...
        ans[b]++;
        ans[lca]--;
        // if parent of lca exists
        if(parent[lca][0]!=-1)
        ans[parent[lca][0]]--;
    }

// ans for every node is sum of subtree for that node
    dfs_for_ans(1,-1,ans);
